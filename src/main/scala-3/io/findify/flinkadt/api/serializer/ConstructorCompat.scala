...
private[serializer] trait ConstructorCompat:
  ...
  final def lookupConstructor[T](cls: Class[T], parameterClasses: Array[Class[?]]): Array[AnyRef] => T =
    ...
    val constructor = try
      cls.getConstructors
        .collectFirst { case c if c.getParameterCount == parameterClasses.length => c }
        .get
